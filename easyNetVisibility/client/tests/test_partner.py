...
class TestPartnerInit(unittest.TestCase):
    def test_init_sets_credentials(self):
        """Test that init properly sets connection parameters"""
        partner.init('http://192.168.1.1', 'admin', 'test_password', False)
        
        self.assertEqual(partner._partner_host, 'http://192.168.1.1')
        self.assertEqual(partner._partner_username, 'admin')
        self.assertEqual(partner._partner_password, 'test_password')
        self.assertEqual(partner._validate_ssl, False)


class TestPartnerDHCPLeases(unittest.TestCase):
    def setUp(self):
        partner.init('http://192.168.1.1', 'admin', 'test_password', False)
    
    @patch('partner._make_request')
    def test_get_dhcp_leases_table_format(self, mock_request):
        ...
        result = partner.get_dhcp_leases()
...
class TestPartnerConnectedDevices(unittest.TestCase):
    def setUp(self):
        partner.init('http://192.168.1.1', 'admin', 'test_password', False)
    ...
    def test_get_connected_devices_success(self, mock_request):
        ...
        result = partner.get_connected_devices()
...
class TestPartnerDiscoverDevices(unittest.TestCase):
    def setUp(self):
        partner.init('http://192.168.1.1', 'admin', 'test_password', False)
    
    @patch('partner.get_connected_devices')
    @patch('partner.get_dhcp_leases')
    def test_discover_devices_from_dhcp_only(self, mock_dhcp, mock_connected):
        ...
        result = partner.discover_devices()
