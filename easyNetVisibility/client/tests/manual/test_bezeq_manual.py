#!/usr/bin/env python3
"""
Manual test script for Bezeq Be router integration.

This script tests the Bezeq router API integration by connecting to a Bezeq
router and displaying discovered devices.

Usage:
    python test_bezeq_manual.py --host http://192.168.1.1 --username admin --password PASSWORD [--no-ssl-verify]

Requirements:
    - Access to a Bezeq Be router
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Test Bezeq Be router API integration'
    )
    parser.add_argument(
        '--host',
        required=True,
        help='Bezeq router URL (e.g., http://192.168.1.1)'
    )
    parser.add_argument(
        '--username',
        default='admin',
        help='Bezeq router admin username (default: admin)'
    )
    parser.add_argument(
        '--password',
        required=True,
        help='Bezeq router admin password'
    )
    parser.add_argument(
        '--no-ssl-verify',
        action='store_true',
    ...
    print("\n" + "="*80)
    print("Bezeq Be Router Integration Test")
    print("="*80)
    ...
    try:
        # Initialize Bezeq integration
        print("Initializing Bezeq router connection...")
        bezeq.init(
            host=args.host,
